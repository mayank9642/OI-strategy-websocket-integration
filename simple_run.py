...
try:
    ...
    logger.info(f"Files in src directory: {os.listdir('src')}")
    
    # Import the fixed strategy
    logger.info("Attempting to import FixedOpenInterestStrategy...")
    ...
    logger.info(f"Strategy created, type: {type(strategy).__name__}")
    ...
    logger.info("Running strategy with force_analysis=True...")
    ...
    if hasattr(strategy, 'active_trade') and strategy.active_trade:
        logger.info(f"Trade taken: {strategy.active_trade}")
    else:
        logger.info("No trade was taken")
